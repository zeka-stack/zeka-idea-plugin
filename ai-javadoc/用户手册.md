...
- 自动检测提交的 Java 文件中缺少 JavaDoc 的元素
- 非覆盖模式：只为没有 JavaDoc 的元素生成，不会覆盖已有注释
- 批量处理：一次性为所有缺少 JavaDoc 的元素生成文档
- 智能过滤：自动过滤已有 JavaDoc 的元素，只处理需要生成的部分
...
- 注重数据隐私的用户可以使用 **Ollama**，但需要本地有足够的计算资源
...
- 通义千问和 Ollama 会自动填充默认地址，一般不需要修改
- 只有使用自定义服务时才需要手动填写
...
- **通义千问**：必须填写，在阿里云控制台获取
- **Ollama**：不需要填写（本地服务无需验证）
- **自定义服务**：根据服务要求决定是否需要

**安全提示**：

- API Key 会加密保存，不会明文显示
- 建议定期更换 API Key，避免泄露
...
- 首次使用建议选择 `qwen-plus` 或 `qwen-turbo`
- 对质量要求高时选择 `qwen-max`
- 模型下拉框支持手动输入，可以输入任何模型名称
