...
IntelliAI Terminal 在 IDEA 的 Terminal 中注入 AI 功能，基于 IntelliAI Engine 统一管理 AI 服务商。
...
1. **重命名包结构**：将 `dev.dong4j.zeka.stack.idea.plugin.terminal` 替换为你的包名
2. **重命名类名**：将所有 `Terminal*` 类重命名为你的类名
...
│   │   │       └── util/
│   │   └── resources/
│   │       ├── dev/dong4j/zeka/stack/idea/plugin/terminal/icons/
│   │       ├── messages/
│   │       └── META-INF/
│   └── test/
├── includes/
