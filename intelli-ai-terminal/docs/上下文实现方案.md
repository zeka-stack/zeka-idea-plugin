...
public class ProjectTypeDetector {
    ...
    /**
     * 从终端视图的当前目录检测项目类型
     * <p>使用 terminalView.getCurrentDirectory() 获取终端当前工作目录，然后转换为 VirtualFile 进行检测
     *
     ...
     */
    @NotNull
    public static ProjectInfo detectFromTerminal(@NotNull TerminalView terminalView,
                                                  @NotNull Project project) {
        String currentDirectory = terminalView.getCurrentDirectory();
        if (currentDirectory == null || currentDirectory.isEmpty()) {
            // 如果无法获取终端当前目录，回退到项目根目录
            VirtualFile baseDir = project.getBaseDir();
            if (baseDir != null) {
                return detect(baseDir);
            }
            return new ProjectInfo(false, false, false, false);
        }

        // 将当前目录路径转换为 VirtualFile
        VirtualFile currentDir = LocalFileSystem.getInstance().findFileByPath(currentDirectory);
        if (currentDir == null) {
            // 如果找不到目录，回退到项目根目录
            VirtualFile baseDir = project.getBaseDir();
            if (baseDir != null) {
...
public class TechStackDetector {
     ...
     */
    @NotNull
    public static TechStack detectFromTerminal(@NotNull TerminalView terminalView,
                                                @NotNull Project project) {
        String currentDirectory = terminalView.getCurrentDirectory();
        if (currentDirectory == null || currentDirectory.isEmpty()) {
            // 如果无法获取终端当前目录，回退到项目根目录
...
public class TerminalContextCollector {
    ...
    /**
     * 从终端视图构建上下文收集器
     * <p>使用 terminalView.getCurrentDirectory() 获取终端当前工作目录，基于该目录进行上下文检测
     *
     * @param project 项目实例
     * @param terminalView 终端视图实例
     */
    public TerminalContextCollector(@NotNull Project project, @NotNull TerminalView terminalView) {
        ...
        if (currentDirectory != null && !currentDirectory.isEmpty()) {
            VirtualFile currentDir = LocalFileSystem.getInstance().findFileByPath(currentDirectory);
            if (currentDir != null) {
                this.projectInfo = ProjectTypeDetector.detect(currentDir);
                this.techStack = TechStackDetector.detect(currentDir);
            } else {
                // 如果找不到目录，回退到项目根目录
                VirtualFile baseDir = project.getBaseDir();
                if (baseDir != null) {
    ...
    public ContextInfo collect() {
        return new ContextInfo(
            ...
            projectInfo,
            techStack,
            terminalView.getCurrentDirectory()
        );
    }
...
public class ContextPromptBuilder {
    ...
    public static String buildUserPrompt(@NotNull TerminalContextCollector.ContextInfo context,
        ...
        // 当前目录
        if (context.currentDirectory != null && !context.currentDirectory.isEmpty()) {
...
// 收集上下文（基于终端当前工作目录）
TerminalContextCollector collector = new TerminalContextCollector(project, terminalView);
...
- 使用 `terminalView.getCurrentDirectory()` 获取终端当前工作目录
- 基于终端当前目录进行上下文检测，而不是项目根目录
- 这样即使终端在项目子目录中，也能正确检测该目录的上下文
...
public class ContextDetectorFactory {
    ...
    public static TerminalContextCollector createCollector(@NotNull Project project,
        ...
        return new TerminalContextCollector(project, terminalView);
    }
}
```
...
public class TerminalAiGenerateAction extends DumbAwareAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        // 收集上下文（基于终端当前目录）
        TerminalContextCollector collector = ContextDetectorFactory.createCollector(project, terminalView);
        TerminalContextCollector.ContextInfo contextInfo = collector.collect();
...
public class CachedContextCollector {
    private static final Map<String, TerminalContextCollector.ContextInfo> cache = new ConcurrentHashMap<>();

    /**
     * 获取或检测上下文信息（带缓存）
     * <p>使用终端当前目录路径作为缓存键，避免重复检测
     *
     * @param project 项目实例
     * @param terminalView 终端视图实例
     * @return 上下文信息
    ...
    public static TerminalContextCollector.ContextInfo getOrDetect(@NotNull Project project,
        ...
        String currentDirectory = terminalView.getCurrentDirectory();
        if (currentDirectory == null || currentDirectory.isEmpty()) {
            // 如果没有当前目录，直接检测并返回（不缓存）
            return new TerminalContextCollector(project, terminalView).collect();
        }

        // 使用当前目录路径作为缓存键
        return cache.computeIfAbsent(currentDirectory, dir -> {
            return new TerminalContextCollector(project, terminalView).collect();
        });
    }
     ...
     */
    public static void clearCache(@NotNull String directoryPath) {
...
public class AsyncContextCollector {
    /**
     * 异步收集上下文信息
     * <p>在后台线程执行上下文检测，避免阻塞 UI 线程
     *
     * @param project 项目实例
     * @param terminalView 终端视图实例
     * @param callback 回调函数，在 EDT 线程执行
     */
...
当前上下文检测在终端处于深层目录时会出现误判：

- `baseDir.findChild(".git")` 只能检查当前目录，不会向上查找
- 终端位于子目录时，Git 根目录的 `.git` 无法命中

**改进方向：**

- 从终端当前目录向上查找根标识（如 `.git` / `pom.xml` / `package.json`）
- 找到根目录后，再在根目录内做项目类型检测

### 10.2 横向分支文件检测（如 k8s yaml）

仅向上查找无法覆盖“兄弟分支”下的配置文件：

- 例如 `k8s/`、`manifests/` 不在当前目录路径上
- 终端所在目录与 YAML 文件是不同分支时，向上查找必然失败

**改进方向：**

- 先定位根目录，再在根目录内做“受限范围扫描”
    - 例如仅扫描 `k8s/`、`manifests/`、`deploy/` 等常见目录
    - 或限制扫描深度与文件数量，避免性能问题
- 或使用索引（如 `FilenameIndex` / `FileBasedIndex`）按扩展名或目录名查询
    - 需处理索引不可用或项目未完全索引的情况
    - 需要明确性能与线程模型（BGT + ReadAction）
