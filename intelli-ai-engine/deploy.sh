...
# intelli-ai-engine 插件发布与部署脚本
# 1) 执行 Gradle 的 publishPlugin 流程
# 2) 将 build/distributions/intelli-ai-engine-{version}.zip 重命名为 eng.zip 并上传到 /var/www/eng-landing/
# 3) 部署 landing.html 到 /var/www/eng-landing/landing.html
# 4) 部署用户手册文档 (docs 目录) 到 /var/www/eng-docs
...
REMOTE_HOST="aliyun"
REMOTE_DIR="/var/www/eng-landing"
REMOTE_LANDING_PATH="$REMOTE_DIR/landing.html"
REMOTE_DOCS_DIR="/var/www/eng-docs"
...
echo "================================"
echo "开始发布与部署 intelli-ai-engine"
echo "================================"
...
if $do_zip; then
    ...
    # 选取最新的 intelli-ai-engine-*.zip
    ZIP_FILE=$(ls -t "$ZIP_DIR"/intelli-ai-engine-*.zip 2>/dev/null | head -n1 || true)
    if [ -z "$ZIP_FILE" ]; then
        echo "错误: 未找到 $ZIP_DIR/intelli-ai-engine-*.zip"
        exit 1
    fi
...
if $do_zip; then
    ...
    echo "  - 下载地址: https://eng.dong4j.site/eng.zip"
fi
if $do_landing; then
    echo "  - HTML: $REMOTE_HOST:$REMOTE_LANDING_PATH"
    echo "  - 访问地址: https://eng.dong4j.site/"
fi
if $do_docs; then
    echo "  - DOCS: $REMOTE_HOST:$REMOTE_DOCS_DIR"
    echo "  - 访问地址: https://eng.dong4j.site/docs"
fi
echo "================================"
