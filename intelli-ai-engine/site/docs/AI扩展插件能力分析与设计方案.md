# IntelliAI Engine 作为 IntelliJ 插件扩展的能力分析与设计方案

## 1. 当前状态分析

### 1.1 现有架构

`intelli-ai-engine` 目前是一个 **Java Library** 项目，通过 Maven 发布，作为库依赖被其他插件使用。
...
✅ **完全可行**，`intelli-ai-engine` 具备作为独立插件发布的所有条件：
...
1. 在 `intelli-ai-engine/src/main/resources/META-INF/` 创建 `plugin.xml`
2. 配置插件基本信息
...
- ✅ 用户只需在 "Settings → Tools → IntelliAI Engine" 管理可用供应商列表
...
2. 用户只需在 "Settings → Tools → IntelliAI Engine" 管理可用供应商列表
...
- 用户已经在 "Settings → Tools → IntelliAI Engine" 中配置了可用供应商列表
...
4. ✅ 创建全局设置页面（"Settings → Tools → IntelliAI Engine"）
5. ✅ 更新构建配置支持插件发布
