...
dependencies {
    ...
    // 编译使用：本地开发时，includeBuild 会自动将 "dev.dong4j:ai-common:1.0.0" 替换为本地项目
    // 发布到市场后，其他开发者可以直接使用 compileOnly("dev.dong4j:ai-common:1.0.0")
    // 运行时依赖通过 copyAiCommonPlugin 任务安装的插件来满足
    compileOnly("dev.dong4j:ai-common:1.0.0")
...
tasks {
    ...
    // 本地开发：构建并复制 ai-common 插件到 sandbox
    val buildAiCommonPlugin = register<Exec>("buildAiCommonPlugin") {
        description = "Build ai-common plugin for local development"
        group = "intellij"

        val aiCommonDir = file("../ai-common")
        workingDir = aiCommonDir
        // 使用 ai-common 项目的 gradlew 来执行构建（先 clean 再 buildPlugin）
    ...
    val copyAiCommonPlugin = register<Copy>("copyAiCommonPlugin") {
        description = "Copy ai-common plugin to sandbox for local development"
        group = "intellij"

        // 先构建 ai-common 插件
        dependsOn(buildAiCommonPlugin)

        // 确保在 prepareSandbox 之后执行（prepareSandbox 会安装 ai-workflow-explainer 插件）
        mustRunAfter("prepareSandbox")

        // 从 ai-common 的构建输出复制插件
        val aiCommonPluginDir = file("../ai-common/build/distributions")
