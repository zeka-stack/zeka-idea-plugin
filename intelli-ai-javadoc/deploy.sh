...
# intelli-ai-javadoc 插件发布与部署脚本
# 1) 执行 Gradle 的 publishPlugin 流程
# 2) 将 build/distributions/intelli-ai-javadoc-{version}.zip 重命名为 aij.zip 并上传到 /var/www/aij-landing/
# 3) 部署 landing.html 到 /var/www/aij-landing/landing.html
# 4) 部署用户手册文档 (docs 目录) 到 /var/www/aij-docs
...
REMOTE_DOCS_DIR="/var/www/aij-docs"
...
echo "================================"
echo "开始发布与部署 intelli-ai-javadoc"
echo "================================"
...
if $do_zip; then
    ...
    # 选取最新的 intelli-ai-javadoc-*.zip
    ZIP_FILE=$(ls -t "$ZIP_DIR"/intelli-ai-javadoc-*.zip 2>/dev/null | head -n1 || true)
    if [ -z "$ZIP_FILE" ]; then
        echo "错误: 未找到 $ZIP_DIR/intelli-ai-javadoc-*.zip"
        exit 1
    fi
...
if $do_zip; then
    ...
    echo "  - 下载地址: https://aij.dong4j.site/aij.zip"
fi
if $do_landing; then
    echo "  - HTML: $REMOTE_HOST:$REMOTE_LANDING_PATH"
    echo "  - 访问地址: https://aij.dong4j.site/"
fi
if $do_docs; then
    echo "  - DOCS: $REMOTE_HOST:$REMOTE_DOCS_DIR"
    echo "  - 访问地址: https://aij.dong4j.site/docs"
fi
echo "================================"
