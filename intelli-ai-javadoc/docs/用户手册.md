# IntelliJ IntelliDoc Assistant 插件操作手册
...
**IntelliDoc Assistant: You Know, for Javaer, Fun**
...
- [GitHub](https://github.com/zeka-stack/zeka-idea-plugin/tree/main/intelli-ai-javadoc)
...
IntelliJ IntelliDoc Assistant 插件是一个功能强大的工具，利用人工智能为您的 Java 代码自动生成高质量的 JavaDoc 注释。插件支持多种 AI
...
**位置**：编辑器底部状态栏 → `AI 服务商切换` 控件（显示插件图标与当前默认服务商）

**功能特点**：

- 随时查看当前默认的 AI 服务商，无需打开设置页面
- 点击后弹出列表，展示所有已通过连接测试的服务商（来源 `availableProviders`）
- 支持在列表中直接切换默认服务商，操作完成后会弹出通知提示

**使用步骤**：

1. 在编辑器窗口底部找到状态栏中的 `AI 服务商切换` 控件，确认当前默认服务商名称
2. 点击控件，弹出下拉列表，列表默认选中当前服务商
3. 从列表中选择目标服务商，插件会自动更新默认配置并刷新状态栏显示

**注意事项**：

- 仅在服务商配置通过「测试连接」后才会出现在列表中
- 切换操作会同步更新 `defaultProviders` 中对应服务商的默认配置
- 若暂未配置任何可用服务商，控件会提示先在设置页面完成验证
...
1. 打开 IntelliJ IDEA
2. 进入 **Settings**（macOS 上为 Preferences）→ **Tools** → **IntelliDoc Assistant**
3. 选择 AI 提供商（推荐通义千问）
4. 配置 API Key 和模型
...
**路径**：Settings → Tools → IntelliDoc Assistant
