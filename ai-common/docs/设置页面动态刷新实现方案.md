...
当用户在 AI Common 设置页面（`Settings → Tools → AI Common`）中添加、删除或清空可用服务商后，第三方插件（如 `ai-javadoc`）的设置页面中的服务商下拉列表需要能够
**动态刷新**，而不需要用户关闭设置页面重新打开。

**问题场景**：

1. 用户打开 `Settings → Tools → AI Javadoc` 设置页面
2. 此时没有可用的服务商，显示警告信息和跳转链接
3. 用户点击链接跳转到 `Settings → Tools → AI Common` 设置页面
4. 在 AI Common 中添加并测试通过一个服务商
5. 返回 AI Javadoc 设置页面，下拉列表应该**自动刷新**，显示新添加的服务商

**原有方案的问题**：

- 下拉列表只在设置页面打开时初始化一次
- 用户在 AI Common 中修改配置后，第三方插件的设置页面无法感知变化
...
- 默认情况下，`PersistentStateComponent` 的配置只在以下时机持久化：
    ...
    2. IDE 关闭时自动保存
- 当用户在 AI Common 设置页面中添加/删除提供商后，如果用户没有点击 "Apply"，配置只存在于内存中
- 第三方插件的设置页面通过监听器刷新时，虽然能看到内存中的最新配置，但如果 IDE 异常关闭，这些配置会丢失
- 更重要的是，如果用户关闭了 AI Common 设置页面（没有点击 Apply），配置变更会丢失
...
2. **用户在 AI Common 中修改配置**：
    - 用户在 AI Common 设置页面中添加/删除/清空提供商
...
```
用户操作                    AI Common 设置页面          AIProviderSettings          JavaDocSettingsPanel
  |                                |                           |                            |
  |-- 打开 AI Javadoc 设置页面 -->|                           |                            |
  |                                |                           |                            |-- 注册监听器 -->
  |                                |                           |<-- addListener() ----------|
