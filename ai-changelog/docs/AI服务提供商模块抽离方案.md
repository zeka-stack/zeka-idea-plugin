...
将 `ai-javadoc` 中与 AI 服务提供商相关的逻辑抽离到独立的 `ai-common` 模块中，实现代码复用，使得 `ai-javadoc` 和 `ai-changelog` 等插件可以共享相同的
AI 服务提供商配置和功能。

### 1.2 设计原则

- **单一职责**：`ai-common` 模块只负责 AI 服务提供商的通用逻辑
- **开闭原则**：对扩展开放，对修改关闭，便于添加新的 AI 提供商
- **依赖倒置**：高层模块（业务逻辑）依赖抽象（接口），不依赖具体实现
- **最小依赖**：`ai-common` 模块尽量少依赖其他模块，保持独立性
...
@State(
    ...
    storages = @Storage("ai-common-provider-config.xml")
)
...
```java
// 在 ai-javadoc 或 ai-changelog 的设置面板中使用
...
### 阶段二：重构 ai-javadoc 使用 ai-common（2-3 天）

#### 2.1 添加依赖

- [ ] 在 `ai-javadoc/build.gradle.kts` 中添加对 `ai-common` 的依赖
- [ ] 更新 `settings.gradle.kts` 包含 `ai-common` 模块

#### 2.2 重构代码

- [ ] 删除 `ai-javadoc` 中的 AI 提供商相关类
...
- [ ] 保持 `generateDocumentation` 方法在 `ai-javadoc` 中作为业务层方法
- [ ] 测试所有功能是否正常

### 阶段三：在 ai-changelog 中集成 ai-common（1-2 天）

#### 3.1 添加依赖

- [ ] 在 `ai-changelog/build.gradle.kts` 中添加对 `ai-common` 的依赖
- [ ] 更新 `settings.gradle.kts` 包含 `ai-common` 模块
...
include("ai-common")
include("ai-javadoc")
include("ai-changelog")
```

在 `ai-javadoc/build.gradle.kts` 中：
